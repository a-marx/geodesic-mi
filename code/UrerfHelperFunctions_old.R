...
GrowUnsupervisedForest <-
  function(X, MinParent = 1, trees = 100,
             ...
             replacement = TRUE, FUN = makeAB, mtry=5,
    ...
    for (treeX in 1:trees) {
      ...
      while (CurrentNode < NextUnusedNode && CurrentNode < StopNode) {
        ...
        best_sparseM <- sparseM
        
        for(n.try in 1:mtry){ # loop mtry (only important for random projections)
          nBest <- 1L
          ...
          # generate new sparse matrix for multiple random projections
          if(splitCrit == "random"){
            sparseM <- do.call(FUN, options)
          }
        } # end loop with multiple tries
        sparseM <- best_sparseM
